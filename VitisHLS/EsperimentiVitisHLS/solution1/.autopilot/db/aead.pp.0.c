...
# 3 "source/aead.c" 2
# 1 "source/crypto_aead.h" 1
# 10 "source/crypto_aead.h"
...
# 1 "source/constants.h" 1
# 8 "source/permutations.h" 2
# 1 "source/printstate.h" 1
# 9 "source/permutations.h" 2
# 1 "source/round.h" 1
...
static inline void ROUND(ascon_state_t* s, uint8_t C) {
  ...
  VITIS_LOOP_39_1: do { } while (0);
}
# 10 "source/permutations.h" 2
...
static inline uint64_t LOADBYTES(const uint8_t* bytes, int n) {
  int i;
  uint64_t x = 0;
  VITIS_LOOP_22_1: for (i = 0; i < n; ++i) x |= ((uint64_t)(bytes[i]) << (56 - 8 * (i)));
  return x;
}
...
static inline void STOREBYTES(uint8_t* bytes, uint64_t x, int n) {
  int i;
  VITIS_LOOP_29_1: for (i = 0; i < n; ++i) bytes[i] = ((uint8_t)((uint64_t)(x) >> (56 - 8 * (i))));
}


static inline uint64_t CLEARBYTES(uint64_t x, int n) {
  int i;
  VITIS_LOOP_35_1: for (i = 0; i < n; ++i) x &= ~((uint64_t)(0xff) << (56 - 8 * (i)));
  return x;
}
# 7 "source/aead.c" 2
...
int crypto_aead_encrypt(
  ...
  *clen = mlen + 16;
  ...
  ascon_state_t s;
  s.x[0] = (((uint64_t)(16 * 8) << 56) | ((uint64_t)(8 * 8) << 48) | ((uint64_t)(12) << 40) | ((uint64_t)(6) << 32));
  s.x[1] = K0;
  s.x[2] = K1;
  s.x[3] = N0;
  s.x[4] = N1;
  VITIS_LOOP_32_1: do { } while (0);
  P12(&s);
  s.x[3] ^= K0;
  s.x[4] ^= K1;
  VITIS_LOOP_36_2: do { } while (0);

  if (adlen) {

    VITIS_LOOP_40_3: while (adlen >= 8) {
      s.x[0] ^= LOADBYTES(ad, 8);
      VITIS_LOOP_42_4: do { } while (0);
      P6(&s);
      ad += 8;
      adlen -= 8;
    }
    ...
    s.x[0] ^= ((uint64_t)(0x80) << (56 - 8 * (adlen)));
    VITIS_LOOP_50_5: do { } while (0);
    P6(&s);
  }

  s.x[4] ^= 1;
  VITIS_LOOP_55_6: do { } while (0);


  VITIS_LOOP_58_7: while (mlen >= 8) {
    s.x[0] ^= LOADBYTES(m, 8);
    STOREBYTES(c, s.x[0], 8);
    VITIS_LOOP_61_8: do { } while (0);
    P6(&s);
    m += 8;
    c += 8;
    mlen -= 8;
  ...
  s.x[0] ^= ((uint64_t)(0x80) << (56 - 8 * (mlen)));
  c += mlen;
  VITIS_LOOP_72_9: do { } while (0);


  s.x[1] ^= K0;
  s.x[2] ^= K1;
  VITIS_LOOP_77_10: do { } while (0);
  P12(&s);
  s.x[3] ^= K0;
  s.x[4] ^= K1;
  VITIS_LOOP_81_11: do { } while (0);
...
int crypto_aead_decrypt(unsigned char* m, unsigned long long* mlen,
  ...
  *mlen = clen - 16;
  ...
  s.x[3] = N0;
  s.x[4] = N1;
  VITIS_LOOP_115_1: do { } while (0);
  P12(&s);
  s.x[3] ^= K0;
  s.x[4] ^= K1;
  VITIS_LOOP_119_2: do { } while (0);

  if (adlen) {

    VITIS_LOOP_123_3: while (adlen >= 8) {
      s.x[0] ^= LOADBYTES(ad, 8);
      VITIS_LOOP_125_4: do { } while (0);
      P6(&s);
      ad += 8;
      adlen -= 8;
    }
    ...
    s.x[0] ^= LOADBYTES(ad, adlen);
    s.x[0] ^= ((uint64_t)(0x80) << (56 - 8 * (adlen)));
    VITIS_LOOP_133_5: do { } while (0);
    P6(&s);
  }

  s.x[4] ^= 1;
  VITIS_LOOP_138_6: do { } while (0);


  clen -= 16;
  VITIS_LOOP_142_7: while (clen >= 8) {
    uint64_t c0 = LOADBYTES(c, 8);
    STOREBYTES(m, s.x[0] ^ c0, 8);
    s.x[0] = c0;
    VITIS_LOOP_146_8: do { } while (0);
    P6(&s);
    m += 8;
    c += 8;
    clen -= 8;
  ...
  s.x[0] |= c0;
  s.x[0] ^= ((uint64_t)(0x80) << (56 - 8 * (clen)));
  c += clen;
  VITIS_LOOP_159_9: do { } while (0);


  s.x[1] ^= K0;
  s.x[2] ^= K1;
  VITIS_LOOP_164_10: do { } while (0);
  P12(&s);
  s.x[3] ^= K0;
  s.x[4] ^= K1;
  VITIS_LOOP_168_11: do { } while (0);
  ...
  int i;
  int result = 0;
  VITIS_LOOP_178_12: for (i = 0; i < 16; ++i) result |= c[i] ^ t[i];
